# [feat] Persist Control Tower approvals

## implementation id
`V1-07A`

## source mapping
- original issue: `V1-07`
- split from: `V1-07`
- source files:
...
## ctx
- component: `Control Tower approval persistence and resume queue`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-07B,V1-12C,V1-13`
...
## goal
> Deliver `V1-07A` by making `Persist Control Tower approvals` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` - persist approval decisions and enqueue resume commands with actor/checksum validation
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0008_v1_approvals.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add approval endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_07a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-07A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-06B2` |
| blocks     | `V1-07B,V1-12C,V1-13` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_07a_*.py -rs --tb=short
