...
## goal
> Deliver `V1-13` by making `Gate plans with reviewer` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` and `domain/transitions.py` - gate plan amendments by reviewer decision and checksum
...
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add plan review endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_13_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Unapproved plan candidates cannot unblock downstream work.
- [ ] Stale checksum approvals are rejected.
...
- model: `Mistral-Large-3 reviewer through registered profile; fake provider in tests`
...
## output contract
```text
Future PR for V1-13 changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-12C` |
| blocks     | `V1-14A,V1-18C` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_13_*.py -rs --tb=short
