# [security] Validate patch policy
...
## goal
> Deliver `V1-15A` by making `Validate patch policy` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_15a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-15A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```

## deps
| relation   | issue             |
| ---------- | ----------------- |
| blocked_by | `V1-17D,V1-14C,V1-00D` |
| blocks     | `V1-15B,V1-15C` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_15a_*.py -rs --tb=short
