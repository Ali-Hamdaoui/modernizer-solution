# [feat] Compute deterministic proof gates
...
## goal
> Deliver `V1-19A` by making `Compute deterministic proof gates` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_19a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-19A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-08A` |
| blocks     | `V1-19B,V1-19C,V1-18G` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_19a_*.py -rs --tb=short
