# [chore] Register V1 pipeline and runner fixtures
...
## goal
> Deliver `V1-00B` by making `Register V1 pipeline and runner fixtures` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/schemas/pipeline_definition.py` and `schemas/runner_profile.py` - register typed V1 route/runner fixture fields without browser path input
...
- [ ] `tests/control_tower/test_v1_00b_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-00B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_00b_*.py -rs --tb=short
