# [feat] Persist ledger during job creation
...
## goal
> Deliver `V1-03B` by making `Persist ledger during job creation` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_03b_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-03B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-03A` |
| blocks     | `V1-04,V1-06A,V1-08A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_03b_*.py -rs --tb=short
