# [feat] Define V1 event type registry
...
## ctx
- component: `V1 event type registry and event replay contracts`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-06A,V1-07A,V1-08A,V1-10,V1-18B`
...
## goal
> Deliver `V1-00C` by making `Define V1 event type registry` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/domain/entities.py` or `application/dto.py` - expose V1 event constants only if registry tests need typed names
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0007_v1_event_types.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
...
- [ ] `tests/control_tower/test_v1_00c_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-00C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-04` |
| blocks     | `V1-06A,V1-07A,V1-08A,V1-10,V1-18B` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_00c_*.py -rs --tb=short
