# [feat] Snapshot sandbox before patch
...
## goal
> Deliver `V1-15B` by making `Snapshot sandbox before patch` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_15b_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-15B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-15A` |
| blocks     | `V1-15C,V1-15E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_15b_*.py -rs --tb=short
