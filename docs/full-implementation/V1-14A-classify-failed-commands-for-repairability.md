# [feat] Classify failed commands for repairability
...
## goal
> Deliver `V1-14A` by making `Classify failed commands for repairability` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_14a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-14A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-13` |
| blocks     | `V1-14B,V1-14C,V1-18E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_14a_*.py -rs --tb=short
