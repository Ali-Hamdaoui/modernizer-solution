# [feat] Persist pending privileged actions
...
## goal
> Deliver `V1-17A` by making `Persist pending privileged actions` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_17a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-17A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-16B` |
| blocks     | `V1-17B,V1-17C,V1-18C` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_17a_*.py -rs --tb=short
