# [feat] Expose proof and report API
...
## goal
> Deliver `V1-19C` by making `Expose proof and report API` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_19c_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-19C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_19c_*.py -rs --tb=short
