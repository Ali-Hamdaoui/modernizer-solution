# [feat] Define orchestrator stage command manifest

## implementation id
`V1-06A`

## source mapping
- original issue: `V1-06`
- split from: `V1-06`
- source files:
...
## ctx
- component: `Control Tower command queue, command manifests, and worker launcher`
- stack: `Python services | SQLite | worker launcher | Maven/JDK policy | pytest`
- related: `V1-06B1,V1-08B`

## current code evidence
- `migration_factory/control_tower/domain/manifests.py` - CommandManifest lacks stage ledger ID, selected JDK ID, profile/catalog checksums, and sandbox refs.
...
## goal
> Deliver `V1-06A` by making `Define orchestrator stage command manifest` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/domain/manifests.py`, `application/commands.py`, and `application/services.py` - define checksum-bound orchestrator stage commands and queue behavior
...
- [ ] `tests/control_tower/test_v1_06a_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Manifest checksum covers ledger/JDK/profile/catalog/sandbox/argv/env refs.
- [ ] Stage launch uses shell=False and route handlers do not execute.
...
## output contract
```text
Future PR for V1-06A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-05` |
| blocks     | `V1-06B1,V1-08B` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_06a_*.py -rs --tb=short
