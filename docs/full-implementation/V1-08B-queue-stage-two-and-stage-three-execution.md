# [feat] Queue Stage Two and Stage Three execution
...
## goal
> Deliver `V1-08B` by making `Queue Stage Two and Stage Three execution` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_08b_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-08B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-08A` |
| blocks     | `V1-12A,V1-19A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_08b_*.py -rs --tb=short
