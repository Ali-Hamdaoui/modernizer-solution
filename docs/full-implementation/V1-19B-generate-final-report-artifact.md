# [feat] Generate final report artifact

## implementation id
`V1-19B`

## source mapping
- original issue: `V1-19`
- split from: `V1-19`
- source files:
...
## ctx
- component: `deterministic proof gates, final report artifact, and proof API`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-19C,V1-18G`

## current code evidence
- `migration_factory/control_tower/domain/states.py` - TargetProofLevel exists, but proof gates do not.
- `migration_factory/control_tower/application/services.py` - command finalization registers terminal artifacts, but no proof computation exists.
...
## goal
> Deliver `V1-19B` by making `Generate final report artifact` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/proof.py` and `services.py` - compute deterministic proof gates and final report artifacts
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0015_v1_proof_reports.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add proof/report endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_19b_*.py` - add focused tests for this issue plus affected regression coverage
...
- model: `none`
- token budget: `optional summary <=6000 input / <=1500 output`
- do NOT touch:
...
## output contract
```text
Future PR for V1-19B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```

## deps
| relation   | issue             |
| ---------- | ----------------- |
| blocked_by | `V1-19A,V1-00D` |
| blocks     | `V1-19C,V1-18G` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_19b_*.py -rs --tb=short
