# [security] Add redaction filtering to context packs

## implementation id
`V1-11C`

## source mapping
- original issue: `V1-11`
- split from: `V1-11`
- source files:
...
## ctx
- component: `context builder, evidence retrieval, redaction, and forbidden path policy`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-12B,V1-14C,V1-16A,V1-19B`

## current code evidence
- `migration_factory/control_tower/application` - no context_packs.py or redaction.py module exists.
- `migration_factory/control_tower` - Artifact and command-output repositories exist and can be used as bounded evidence sources.
...
## goal
> Deliver `V1-11C` by making `Add redaction filtering to context packs` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/context_packs.py`, `queries.py`, and `redaction.py` - persist bounded redacted context pack manifests and evidence retrievers
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0011_v1_context_packs.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
...
- [ ] `tests/control_tower/test_v1_11c_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Context packs persist evidence refs/bounds/redaction/checksum.
- [ ] Retrievers reject absolute paths and over-limit windows.
...
## output contract
```text
Future PR for V1-11C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-11B` |
| blocks     | `V1-12B,V1-14C,V1-16A,V1-19B` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_11c_*.py -rs --tb=short
