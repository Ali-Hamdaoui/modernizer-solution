...
## ctx
- component: `runner profile health and Java 11/17/21 readiness`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-06B1,V1-08B,V1-18A`

## current code evidence
- `tests/control_tower/test_runner_profile_schema.py` - tests existing raw-path runner profile schema.
- `web/control-tower/app/jobs/new/CreateDiagnosticJobForm.tsx` - no runner readiness badges exist.
...
## goal
> Deliver `V1-05` by making `Validate runner JDK readiness` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` - add fakeable Java 11/17/21 and Maven readiness checks from backend-owned refs
...
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add runner readiness endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_05_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Java 11/17/21 and Maven checks are independently reported.
...
## output contract
```text
Future PR for V1-05 changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-04` |
| blocks     | `V1-06B1,V1-08B,V1-18A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_05_*.py -rs --tb=short
