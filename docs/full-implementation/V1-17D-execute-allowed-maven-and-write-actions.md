# [feat] Execute allowed Maven and write actions
...
## goal
> Deliver `V1-17D` by making `Execute allowed Maven and write actions` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_17d_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-17D changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-17C` |
| blocks     | `V1-15A,V1-18C` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_17d_*.py -rs --tb=short
