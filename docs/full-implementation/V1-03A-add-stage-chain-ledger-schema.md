# [feat] Add stage-chain ledger schema

## implementation id
`V1-03A`

## source mapping
- original issue: `V1-03`
- split from: `V1-03`
- source files:
...
## ctx
- component: `Control Tower SQLite persistence and stage-chain ledger`
- stack: `Python | SQLite | FastAPI | pytest | TypeScript contracts where applicable`
- related: `V1-03B,V1-04,V1-08A,V1-19A`

## current code evidence
- `migration_factory/control_tower/infrastructure/sqlite/migrations/0001_foundation.sql` - contains stage_runs but no stage_chain_ledger.
- `migration_factory/control_tower/infrastructure/sqlite/repositories.py` - has StageRun repository methods but no stage-chain ledger repository.
...
## goal
> Deliver `V1-03A` by making `Add stage-chain ledger schema` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` - persist/read stage-chain ledger rows only for slice-owned behavior
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0007_v1_stage_chain_ledger.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
...
- [ ] `tests/control_tower/test_v1_03a_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Migration follows 0006 and ledger rows are append-only.
- [ ] Job creation persists three checksum-bound stage rows.
...
## output contract
```text
Future PR for V1-03A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-02` |
| blocks     | `V1-03B,V1-04,V1-08A,V1-19A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_03a_*.py -rs --tb=short
