# [security] Define redaction and forbidden-path baseline
...
- related: `V1-11C,V1-16A,V1-17B,V1-15A,V1-19B`
...
## goal
> Deliver `V1-00D` by making `Define redaction and forbidden-path baseline` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/redaction.py` - define shared redaction for secrets, paths, prompts, env values, and model refs
...
- [ ] `tests/control_tower/test_v1_00d_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-00D changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-10` |
| blocks     | `V1-11C,V1-16A,V1-17B,V1-15A,V1-19B` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_00d_*.py -rs --tb=short
