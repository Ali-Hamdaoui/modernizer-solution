...
## goal
> Deliver `V1-15E` by making `Roll back failed repair` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_15e_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-15E changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-15D` |
| blocks     | `V1-19A,V1-18E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_15e_*.py -rs --tb=short
