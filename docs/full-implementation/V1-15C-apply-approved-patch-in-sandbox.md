# [feat] Apply approved patch in sandbox
...
## goal
> Deliver `V1-15C` by making `Apply approved patch in sandbox` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_15c_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-15C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-15B` |
| blocks     | `V1-15D,V1-15E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_15c_*.py -rs --tb=short
