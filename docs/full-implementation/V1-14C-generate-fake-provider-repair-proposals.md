# [feat] Generate fake-provider repair proposals

## implementation id
`V1-14C`

## source mapping
- original issue: `V1-14`
- split from: `V1-14`
- source files:
...
## goal
> Deliver `V1-14C` by making `Generate fake-provider repair proposals` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/repairs.py` and `services.py` - classify failures, enforce repair limits, and persist fake-provider repair proposals
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0013_v1_repairs.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add repair projection endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_14c_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Repairability reasons and attempt limits are deterministic.
- [ ] Fake proposals are audited and never apply patches.
...
- token budget: `20000 input / 5000 output`
- do NOT touch:
...
## output contract
```text
Future PR for V1-14C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```

## deps
| relation   | issue             |
| ---------- | ----------------- |
| blocked_by | `V1-14B,V1-11C,V1-10` |
| blocks     | `V1-15A,V1-18E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_14c_*.py -rs --tb=short
