...
- related: `V1-10,V1-11A,V1-12B,V1-14C,V1-16A,V1-18D`
...
## goal
> Deliver `V1-09` by making `Register Azure model profiles` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` - register redacted model profiles and fake-provider readiness from env refs
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0009_v1_model_profiles.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add `/v1/model-profiles` and capabilities; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_09_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Model config is stored as env refs only.
- [ ] Live Azure checks are opt-in and fake provider tests are default.
...
## output contract
```text
Future PR for V1-09 changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-00C` |
| blocks     | `V1-10,V1-11A,V1-12B,V1-14C,V1-16A,V1-18D` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_09_*.py -rs --tb=short
