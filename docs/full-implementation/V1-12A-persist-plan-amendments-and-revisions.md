# [feat] Persist plan amendments and revisions
...
## goal
> Deliver `V1-12A` by making `Persist plan amendments and revisions` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_12a_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-12A changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-11A` |
| blocks     | `V1-12B,V1-12C,V1-13` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_12a_*.py -rs --tb=short
