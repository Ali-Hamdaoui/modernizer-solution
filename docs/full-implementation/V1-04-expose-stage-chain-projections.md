...
## goal
> Deliver `V1-04` by making `Expose stage chain projections` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/dto.py` and `queries.py` - project ordered redacted stages from ledger rows
...
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add `GET /v1/jobs/{job_id}/stages`; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_04_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Stage API returns ordered redacted ledger DTOs.
- [ ] Unknown job/stage errors are deterministic.
...
## output contract
```text
Future PR for V1-04 changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-03B` |
| blocks     | `V1-05,V1-18B,V1-19A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_04_*.py -rs --tb=short
