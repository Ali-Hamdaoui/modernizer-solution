...
## ctx
- component: `model profile registry and model invocation audit`
- stack: `SQLite | FastAPI | audited model gateway | structured outputs | pytest`
- related: `V1-00D,V1-11A,V1-12B,V1-14C,V1-16A,V1-18D`

## current code evidence
- `migration_factory/control_tower/schemas/runner_profile.py` - ai_profile is currently only a profile_id reference.
- `migration_factory/control_tower/adapters/fastapi/app.py` - no model profile or model audit endpoints exist.
...
## goal
> Deliver `V1-10` by making `Audit model invocations` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py`, `dto.py`, and `queries.py` - audit model invocations and expose read-only activity projections
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0010_v1_model_invocations.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add model activity endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_10_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Model audit rows record refs/profile/provider/tokens/redacted summaries.
- [ ] Raw prompts/secrets/deployment IDs are absent from public DTOs.
...
- model: `fake provider only by default; registered live provider only when explicitly allowed`
- token budget: `none`
- do NOT touch:
...
## output contract
```text
Future PR for V1-10 changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-09` |
| blocks     | `V1-00D,V1-11A,V1-12B,V1-14C,V1-16A,V1-18D` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_10_*.py -rs --tb=short
