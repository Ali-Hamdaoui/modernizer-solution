# [feat] Add plan amendment preview API/UI
...
- `migration_factory/control_tower` - no approvals table, ApprovalRecord, approval endpoints, or approval UI exist.
...
## goal
> Deliver `V1-12C` by making `Add plan amendment preview API/UI` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_12c_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-12C changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_12c_*.py -rs --tb=short
