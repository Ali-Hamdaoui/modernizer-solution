# [feat] Persist repair attempts and limits
...
## goal
> Deliver `V1-14B` by making `Persist repair attempts and limits` concrete in the files listed below, with focused tests and no adjacent V1 behavior.
...
- [ ] `tests/control_tower/test_v1_14b_*.py` - add focused tests for this issue plus affected regression coverage
...
## output contract
```text
Future PR for V1-14B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```
...
| ---------- | ----------------- |
| blocked_by | `V1-14A` |
| blocks     | `V1-14C,V1-15A,V1-18E` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_14b_*.py -rs --tb=short
