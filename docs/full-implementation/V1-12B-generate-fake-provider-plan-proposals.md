# [feat] Generate fake-provider plan proposals

## implementation id
`V1-12B`

## source mapping
- original issue: `V1-12`
- split from: `V1-12`
- source files:
...
## goal
> Deliver `V1-12B` by making `Generate fake-provider plan proposals` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/services.py` - persist plan amendments, revisions, fake-provider proposals, and previews
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0012_v1_plan_amendments.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add plan preview endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_12b_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Plan revisions are ordered/checksum-bound/immutable after decision.
- [ ] Preview writes no source, sandbox, or plan state.
...
## output contract
```text
Future PR for V1-12B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```

## deps
| relation   | issue             |
| ---------- | ----------------- |
| blocked_by | `V1-12A,V1-11C,V1-10` |
| blocks     | `V1-12C,V1-13` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_12b_*.py -rs --tb=short
