# [security] Validate action policy and checksums

## implementation id
`V1-17B`

## source mapping
- original issue: `V1-17`
- split from: `V1-17`
- source files:
...
## ctx
- component: `privileged typed actions, policy, approval, and execution queue`
- stack: `Python services | SQLite | worker launcher | Maven/JDK policy | pytest`
- related: `V1-17C,V1-17D,V1-15A`

## current code evidence
- `migration_factory/control_tower/domain/commands.py` - command states exist, but privileged action states do not.
- `web/control-tower` - no separate privileged action cards exist.
...
## goal
> Deliver `V1-17B` by making `Validate action policy and checksums` concrete in the files listed below, with focused tests and no adjacent V1 behavior.

## scope
- [ ] `migration_factory/control_tower/application/actions.py` and `services.py` - persist, validate, approve/reject, and execute only typed privileged actions
- [ ] `migration_factory/control_tower/infrastructure/sqlite/migrations/0014_v1_privileged_actions.sql` - add only append-only persistence required by this issue; never edit applied migrations 0001-0006
- [ ] `migration_factory/control_tower/adapters/fastapi/app.py` - add privileged action endpoints; route handlers must not execute worker/model/write behavior directly
- [ ] `tests/control_tower/test_v1_17b_*.py` - add focused tests for this issue plus affected regression coverage
...
## acceptance
- [ ] Pending actions persist type/args/checksum/policy/approval state.
- [ ] Only typed Maven/write actions execute; shell is rejected.
...
## output contract
```text
Future PR for V1-17B changes only issue-owned implementation files, includes focused and affected regression evidence, and leaves unrelated deleted docs untouched.
```

## deps
| relation   | issue             |
| ---------- | ----------------- |
| blocked_by | `V1-17A,V1-00D` |
| blocks     | `V1-17C,V1-17D,V1-15A` |

## test plan
```powershell
py -m pytest -q tests/control_tower/test_v1_17b_*.py -rs --tb=short
