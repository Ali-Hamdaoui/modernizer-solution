# F15-JOB-011 — Add SQLite migration for phase gates
...
Create the database schema for gate state.
...
- [ ] Add `0039_v2_phase_gates.sql`.
- [ ] Create `v2_phase_gates`.
- [ ] Add indexes for job/stage/status/open uniqueness.
...
- [ ] Open gate uniqueness is enforced.
- [ ] Rollback path is not required but migration is idempotent in test harness.

## Suggested tests

- `test_v2_phase_gate_migration.py`
