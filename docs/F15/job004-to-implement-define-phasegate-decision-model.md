# F15-JOB-004 — Define PhaseGate decision model
...
Create append-only decision records for all user/chatbot gate actions.
...
- [ ] Add decision entity.
- [ ] Require gate_id, action, expected_gate_checksum, idempotency_key.
- [ ] Store result command/gate/revision ids.
...
- [ ] Different payload under same idempotency key is rejected.
- [ ] Decision log is append-only.

## Suggested tests

- `test_v2_gate_idempotency.py`
