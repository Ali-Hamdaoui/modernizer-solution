# F15-JOB-012 — Add SQLite migration for gate decisions
...
Create append-only gate decision persistence.
...
- [ ] Create `v2_phase_gate_decisions`.
- [ ] Add idempotency uniqueness.
- [ ] Add decision result columns.
...
- [ ] Two identical idempotent actions return existing result.
- [ ] Different action with same key rejected.
- [ ] Decision rows are not overwritten.
