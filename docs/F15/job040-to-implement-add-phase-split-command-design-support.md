# F15-JOB-040 — Add phase-split command design support
...
Prepare runner interface for analysis-only/planning-only execution.
...
- [ ] Add CLI/manifest design doc or initial arguments.
- [ ] Map phase outputs to artifact refs.
- [ ] Avoid deep graph interrupt migration initially.
...
- [ ] Analysis phase can be run alone in tests or fake runner.
- [ ] Planning phase can consume accepted analysis revision.
- [ ] No transform happens before approval.

## Suggested tests

- `test_v2_phase_split_command_contract.py`
