# F15-JOB-071 — Add model fallback behavior
...
Keep UI usable if LLM is unavailable without making it unsafe.
...
- [ ] Fallback can show available actions.
- [ ] Fallback cannot invent explanation beyond evidence.
- [ ] State-changing requests need explicit action.
...
- [ ] LLM timeout returns deterministic gate card.
- [ ] No fake summary.
- [ ] Actions remain backend validated.

## Suggested tests

- `test_v2_assistant_gate_fallback.py`
