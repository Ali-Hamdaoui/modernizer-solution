...
| Foundation 0 skipped | Stage 4 or repair work builds on reachable Copilot paths and provider-leaking contracts | Treat FND-01 through FND-06 as blockers before MVP-A and MVP-B implementation. |
| Copilot runtime remains reachable | Prohibited CLI/SDK/report/repair behavior can execute despite product docs | Quarantine defaults, graph routes, preflight probes, report generation, TUI probes, and agent wrappers; add reachability tests. |
| Azure OpenAI internals leak through Foundry labels | Public contracts still expose `azure_openai`, env refs, deployments, or fallback semantics | Hide all provider internals behind the Azure AI Foundry adapter and add recursive DTO/frontend leakage tests. |
...
| Context packs are not content-bound | Model audit cannot prove the exact redacted input or policy used | Canonical SHA-256 checksums, policy versions, and invocation-record bindings are required before model-backed repair. |
