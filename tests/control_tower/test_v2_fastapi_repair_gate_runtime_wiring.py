...
def _seed_manual_policy(conn: sqlite3.Connection, job_id: str) -> None:
    seed_job(conn, job_id=job_id)
    with SqliteUnitOfWork(conn) as uow:
        policy = RunPolicy(stage_continuation_policy=StageContinuationPolicy.MANUAL)
        uow.run_configurations.insert(
            RunConfigurationRecord(
                run_configuration_id="rc-manual",
                job_id=job_id,
                schema_version="1",
                ...
                payload_json="{}",
                payload_checksum="sha256:manual-policy",
...
def test_fastapi_create_app_repair_gate_callback_creates_repair_review_gate(tmp_path: Path) -> None:
    ...
    _seed_manual_policy(conn, job_id)
