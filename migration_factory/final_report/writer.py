...
def generate_final_migration_report(state: dict[str, Any]) -> FinalReportResult:
    ...
    full_source = _object_or_empty(state.get("full_migration_source_stack")) or source_stack
    full_target = (
        _object_or_empty(state.get("full_migration_target_stack"))
        or _completed_target_stack(pipeline_history)
        or target_stack
    )
    ...
    report_payload = {
        ...
        "warnings": _dedupe_strings([str(item) for item in [*list(state.get("warnings", []) or []), *boot4_warnings]]),
        "blockers": _dedupe_strings([str(item) for item in _list(state.get("blockers"))]),
        "errors": _dedupe_strings([str(item) for item in _list(state.get("errors"))]),
        "limitations": [
...
def _build_markdown_summary(payload: dict[str, Any]) -> str:
    ...
    raw_change_summary = payload.get("change_summary")
    if isinstance(raw_change_summary, dict):
        change_summary = [str(item) for item in list(raw_change_summary.get("changes", []) or [])]
        description = str(raw_change_summary.get("description") or "")
        if description and description not in change_summary:
            change_summary.insert(0, description)
    elif isinstance(raw_change_summary, list):
        change_summary = [str(item) for item in raw_change_summary]
    else:
        change_summary = []
    ...
    blockers = [str(item) for item in list(payload.get("blockers", []) or []) if str(item)]
    errors = [str(item) for item in list(payload.get("errors", []) or []) if str(item)]
    log_paths = dict(payload.get("log_paths", {}) or {})
    ...
    lines = [
        ...
        f"| **Transform Status** | **{payload.get('transform_status', '') or 'not_captured'}** |",
        f"| **Build Status** | **{payload.get('build_status', '') or 'not_captured'}** |",
        f"| **Test Status** | **{payload.get('test_status', '') or 'not_captured'}** |",
    ...
    if pipeline_history:
        ...
        for stage in pipeline_history:
            ...
            status = stage_row.get("chain_status", stage_row.get("status", "not_captured"))
            lines.append(
        ...
        for stage in pipeline_history:
            ...
            lines.append(
                ...
                f"to **{_stack_path_text(target)}**, with overall stage status **{status}**."
            )
            events = stage_row.get("events")
            if isinstance(events, list) and events:
                event_text = "; ".join(
                    f"{dict(event).get('type', '')}={dict(event).get('status', '')}"
                    for event in events[:5]
                    if isinstance(event, dict)
                )
                if event_text:
                    lines.append(f"  - Events: {event_text}")
            stage_warnings = [str(item) for item in list(stage_row.get("warnings", []) or []) if str(item)]
            stage_errors = [str(item) for item in list(stage_row.get("errors", []) or []) if str(item)]
            if stage_warnings:
                lines.append(f"  - Warnings: {'; '.join(stage_warnings[:5])}")
            if stage_errors:
                lines.append(f"  - Errors: {'; '.join(stage_errors[:5])}")
    else:
    ...
    if warnings or blockers or errors:
        lines.extend(["", "## 12. Warnings And Errors", ""])
        if warnings:
    ...
    lines.extend([
        ...
        "| Field | Value |",
        "|---|---|",
        f"| **Total duration** | **`{_format_duration(timing.get('total_duration_seconds'))}`** |",
        f"| Timing report | `{timing.get('timing_report', '')}` |",
        f"| Timing summary | `{timing.get('timing_summary', '')}` |",
    ])
    if log_paths:
        lines.extend(["", "## 14. Log References", "", "| Log | Path |", "|---|---|"])
        for name, ref in sorted(log_paths.items()):
            if ref:
                lines.append(f"| `{name}` | `{ref}` |")
    artifacts = dict(payload.get("artifact_refs", {}) or {})
    lines.extend(["", "## 15. Related Artifacts", "", "| Artifact | Path |", "|---|---|"])
    for name in (
        "approval_decision",
        "approved_plan_lock",
        "transformation_execution_plan",
...
def _completed_target_stack(pipeline_history: list[dict[str, Any]]) -> dict[str, Any]:
    for stage in reversed(pipeline_history):
        if not isinstance(stage, dict):
            continue
        status = str(stage.get("chain_status") or stage.get("status") or "")
        if status in {"PASS", "completed", "succeeded", "TRANSFORM_APPLIED_IN_SANDBOX"}:
            target = _object_or_empty(stage.get("target_stack"))
            if target:
                return target
    return {}
...
def _latest_completed_stage(pipeline_history: list[dict[str, Any]]) -> int:
    ...
    for stage in reversed(pipeline_history):
        if not isinstance(stage, dict):
            continue
        idx = _int_or_zero(stage.get("stage_index"))
        if idx and fallback == 0:
            fallback = idx
        status = str(stage.get("chain_status") or stage.get("status") or "")
        if idx and status in {"PASS", "completed", "succeeded", "TRANSFORM_APPLIED_IN_SANDBOX"}:
            return idx
    return fallback
...
def _stack_change_text(label: str, source_value: Any, target_value: Any) -> str:
    source = str(source_value or "").strip()
    target = str(target_value or "").strip()
    if not source or not target or source == target:
        return ""
    return f"{label} upgrade: {source} -> {target}"
...
def _change_summary(
    ...
    if source_stack or target_stack:
        for label, key in (
            ...
            change = _stack_change_text(label, source_stack.get(key), target_stack.get(key))
            if change:
...
def _timing_context(
    ...
    aggregate_duration = sum(
        duration
        for duration in (_float_or_none(stage.get("duration_seconds")) for stage in pipeline_history if isinstance(stage, dict))
        if duration is not None
    )
    stage_timings = [
        {
            "stage_index": stage.get("stage_index", ""),
            "status": stage.get("chain_status", stage.get("status", "")),
            "duration_seconds": duration,
        }
        for stage in pipeline_history
        if isinstance(stage, dict)
        for duration in [_float_or_none(stage.get("duration_seconds"))]
        if duration is not None
    ]
    return {
